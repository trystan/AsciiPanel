...
public class AsciiPanel extends JPanel {
    ...
    /**
     * The width of the AsciiPanel in characters.
     */
    private final int widthInCharacters;

    /**
     * The height of the AsciiPanel in characters.
     */
    private final int heightInCharacters;

    /**
     * The two-dimensional array of AsciiCharacterData objects that represent the characters and their properties.
     */
    private final AsciiCharacterData[][] characters;

    /**
     * The offscreen buffer used for double buffering.
     */
    private Image offscreenBuffer;

    /**
     * The graphics context for the offscreen buffer.
     */
    ...
    /**
     * The name of the terminal font file used to render the glyphs.
     */
    private String terminalFontFile = "cp437_9x16.png";

    /**
     * The default background color of the AsciiPanel.
     */
    private Color defaultBackgroundColor;

    /**
     * The default foreground color of the AsciiPanel.
     */
    ...
    /**
     * The image that contains all the glyph images.
     */
    private BufferedImage glyphSprite;

    /**
     * The array of glyph images, one for each character.
     */
    private BufferedImage[] glyphs;

    /**
     * The previous characters and their properties.
     */
    private AsciiCharacterData[][] previousCharacters;

    /**
     * The AsciiFont used to render the characters.
     */
    ...
    /**
     * Loads the glyph images from the terminal font file. If the file cannot be found, prints an error message to
     * standard error. This method is called internally by the constructor.
     */
    ...
    /**
     * Clears a rectangular section of the screen with the specified AsciiCharacterData and whatever the default
     * foreground and background colors are. The cursor position will not be modified.
     *
    ...
    public AsciiPanel clear(AsciiCharacterData characterData, int x, int y, int width, int height) {
        ...
        if (width < 1) throw new IllegalArgumentException("width " + width + " must be greater than 0.");

        if (height < 1) throw new IllegalArgumentException("height " + height + " must be greater than 0.");
    ...
    /**
     * Writes an AsciiCharacterData object to the specified position on the AsciiPanel. The cursor's position is updated
     * accordingly, but the default foreground or background colors are not.
     *
     * @param characterData the AsciiCharacterData object to write
     * @param x             the distance from the left to begin writing from, must be within [0,
     *                      {@link #getWidthInCharacters()})
     * @param y             the distance from the top to begin writing from, must be within [0,
     *                      {@link #getHeightInCharacters()})
     * @return {@code this} AsciiPanel object for convenient chaining of method calls
     * @throws IllegalArgumentException if the character value of the AsciiCharacterData is outside the range, or if x
     *                                  or y is outside the valid range
     */
    ...
    /**
     * Returns the height of the terminal measured in characters. A standard terminal is 24 characters high.
     *
    ...
    /**
     * Returns the width of the terminal measured in characters. A standard terminal is 80 characters wide.
     *
    ...
    /**
     * Returns the X position of the cursor, which represents the distance from the left edge of the screen where new
    ...
    /**
     * Returns the Y position of the cursor, which represents the distance from the top edge of the screen where new
     * text will be written.
     *
    ...
    /**
     * /** Sets the position of the text cursor to the specified x and y coordinates. The position is measured in
     * character cells, with the origin (0,0) in the upper-left corner. The x coordinate should be within the bounds of
     * the screen, where 0 is the leftmost column and the maximum value is one less than the width of the screen in
     * characters. The y coordinate should also be within the bounds of the screen, where 0 is the top row and the
    ...
    /**
     * Returns the default background color that is used when writing new text. If no explicit background color is
     * specified for a character or area of text, this color is used.
     *
    ...
    public void setDefaultBackgroundColor(Color defaultBackgroundColor) {
        if (defaultBackgroundColor == null) throw new NullPointerException("defaultBackgroundColor must not be null.");

        this.defaultBackgroundColor = defaultBackgroundColor;
    }

    /**
     * Returns the default foreground color that is used when writing new text. If no explicit foreground color is
     * specified for a character or area of text, this color is used.
     *
    ...
    /**
     * Sets the default foreground color that is used when writing new text. This color will be used for all new text
     * unless an explicit foreground color is specified for a character or area of text.
     *
    ...
    public void setDefaultForegroundColor(Color defaultForegroundColor) {
        if (defaultForegroundColor == null) throw new NullPointerException("defaultForegroundColor must not be null.");
    ...
    /**
     * Returns the two-dimensional array of AsciiCharacterData objects that are currently written. The array represents
     * the characters that have been written so far.
     *
    ...
    public void paint(Graphics g) {
        ...
        for (int x = 0; x < widthInCharacters; x++) {
            for (int y = 0; y < heightInCharacters; y++) {
                ...
                if (previousCharacterData != null && newCharacterData.backgroundColor == previousCharacterData.backgroundColor && newCharacterData.foregroundColor == previousCharacterData.foregroundColor && newCharacterData.character == previousCharacterData.character)
                    continue;
    ...
    /**
     * Clears the entire screen with the specified character and whatever the default foreground and background colors
     * are.
     *
    ...
    /**
     * Clear the entire screen with the specified character and specified foreground and background colors. This method
     * will clear the entire screen with the specified character and foreground and background colors. If the foreground
     * or background colors are null, the default colors will be used instead.
     *
     * @param character  The character to write.
     * @param foreground The foreground color or null to use the default.
     * @param background The background color or null to use the default.
    ...
    /**
     * Clears a rectangular section of the screen with the specified character and whatever the default foreground and
     * background colors are. The cursor position will not be modified.
     *
     * @param character the character to write
     ...
     * <p>
     * This method updates the cursor's position after writing the character. To write a character at a specific
     * position on the screen, use the overloaded write method that takes in the x and y coordinates.
     *
     * @param character the character to write
     * @return {@code this} AsciiPanel object for convenient chaining of method calls
     */
    ...
    /**
     * Writes a character to the screen at the specified position. This method updates the cursor's position.
     *
     * @param character the character to write
     * @param x         the distance from the left to begin writing from
     * @param y         the distance from the top to begin writing from
    ...
    /**
     * Write a string to the cursor's position. This updates the cursor's position to the end of the written string.
     *
     ...
     * @param x          the distance from the left to begin writing from (must be within range [0,
     *                   {@link #getWidthInCharacters()})
     * @param y          the distance from the top to begin writing from (must be within range [0,
     *                   {@link #getHeightInCharacters()})
     ...
     * @throws IllegalArgumentException if x or y is out of range, or if the specified string extends beyond the right
     *                                  edge of the panel
     ...
     * @throws IllegalArgumentException if the length of the string exceeds the available space to write to based on the
    ...
    /**
     * Writes a string at the current cursor position with the specified foreground and background colors. This updates
     * the cursor's position to the end of the string.
     *
     * @param string     the string to write (must not be null)
     * @param foreground the foreground color, or null to use the default
     * @param background the background color, or null to use the default
    ...
    /**
     * Writes the specified string to the specified position. The method updates the cursor's position to the end of the
     * written string.
     *
     * @param string the string to write, must not be null
    ...
    /**
     * Writes the specified string to the specified position with the specified foreground color. This method updates
    ...
    /**
     * Writes a string to the center of the panel at the specified row, using the default foreground and background
     * colors.
     *
     * @param string the string to write (must not be null)
     ...
     * @return {@code this} AsciiPanel object for convenient chaining of method calls
     * @throws NullPointerException     if the string argument is null
     * @throws IllegalArgumentException if the specified string is wider than the panel or if the specified row is
     *                                  outside the panel's height
     */
    ...
    /**
     * Writes a string to the center of the panel at the specified y position with the specified foreground color. This
     * method updates the cursor's position but not the default foreground color.
     *
     * @param string     the string to write (must not be null)
     * @param y          the distance from the top to begin writing from (must be within the range [0,
     *                   heightInCharacters))
     * @param foreground the foreground color, or null to use the default
     * @return {@code this} AsciiPanel object for convenient chaining of method calls
     * @throws NullPointerException     if the string argument is null
     * @throws IllegalArgumentException if the specified string is too long to fit within the panel, or if y is out of
     *                                  range
     */
    ...
    /**
     * Writes a string to the center of the panel at the specified y position with the specified foreground and
     * background colors. This updates the cursor's position but not the default colors.
     *
     * @param string     the string to write (must not be null)
     * @param y          the distance from the top to begin writing from
     * @param foreground the foreground color, or null to use the default
     ...
     * @return {@code this} AsciiPanel object for convenient chaining of method calls
     * @throws NullPointerException     if the string argument is null
     * @throws IllegalArgumentException if the length of the string is greater than the width of the panel, or if the y
    ...
    public void withEachTile(int left, int top, int width, int height, TileTransformer transformer) {
        for (int x0 = 0; x0 < width; x0++) {
            for (int y0 = 0; y0 < height; y0++) {
                ...
                if (x < 0 || y < 0 || x >= widthInCharacters || y >= heightInCharacters) continue;
